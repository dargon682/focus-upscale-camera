...
- **现代数码相机前端（0.6.000 / 0.8.10）**：墨黑机体配色 + 琥珀机内 HUD 读数、机械滚花快门外观、四角琥珀对焦框、等宽参数排版、全局深色系统一；0.8.10 起引入 M3 分层曲面与按压波纹，详见上文「前端全面重设计」。
- **统一签名（0.6.100）**：debug/release 均使用固定 `keystores/dargon.keystore`（alias `dargon`）签名，相同签名 + 更高版本号即可覆盖安装，无需卸载旧版。
- **检查更新**：设置页「检查更新」从 GitHub（五镜像源自动回退）拉取 `version.json`，有新版本即以**应用内对话框 + 实时进度（速度/剩余时间）**下载，支持**断点续传、多线程分片、完整性+签名校验、指数退避重试、后台/通知栏下载**，下载完成自动进安装确认，失败显示错误码诊断并可一键重试。
...
- **前端全面重设计（0.8.10）**：引入 Material 3 风格分层曲面色板（surface-s0~s3）、按压波纹反馈（HUD 按钮 / 主操作按钮 / 模式切换）、全新机械快门（外缘投影 + 琥珀取景环）、面板卡机件高光与底部投影、底部控制栏由机体色向墨黑渐变、HUD 数值按键精修、设置页「检查更新」升级为琥珀主按钮，视觉与交互双提升。
...
├── apk/photo-tool-v0.8.10.apk         # 已签名的 0.8.10 安装包（v0.8.10）
└── app/src/main
