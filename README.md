...
一个基于 Android CameraX 的拍照工具，支持自动对焦、点按二次对焦、长按锁定对焦/曝光、单张拍照、定时自拍、多帧连拍超分辨率合成、曝光补偿 EV、前后摄切换与闪光灯控制，内置取景构图辅助（九宫格 / 水平仪）、数字变焦、画质档位、历史相册与原图对比，所有功能可通过设置页开关配置，并在设置页支持线上检查更新。
...
├── dist/app-release-v2.1.apk          # 已签名的 release 安装包（同时随 GitHub Release 分发）
└── app/src/main
